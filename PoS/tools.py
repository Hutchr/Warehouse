...
from inventory_manager.models import PaymentMethod
from .models import RetailOrder, Order_status
...
def return_item_qty_change(order_item, qty=None ):
    ...
    create_order = RetailOrder.objects.create(order_type=order_item.order.order_type,
                                                       ...
                                                       status = Order_status.objects.get(id=8),
...
def order_transcation(request, order):
    get_order_type = order.order_type

def order_change_costumer(request, retail_order):
    get_order_status = retail_order.status.id
    print(retail_order.order_type)

    if get_order_status in [7,8,9]:
        retail_order.status = Order_status.objects.get(id=1)
    ...
    if retail_order.order_type == 'e' or retail_order.order_type == 'r':
        ...
        retail_order.costumer_account.balance -= retail_order.value
...
def close_order(request, order):
    if order.order_type == 'r' or order.order_type == 'e':
        if order.value == order.discount:
            print('here')
            order.paid_value = order.value
            order.save()
            order.costumer_account.paid_value += order.value
            order.costumer_account.balance -= order.value
            order.costumer_account.save()
        else:
            if int(order.status.id) in [7,8]:
                pass
            else:
                order.costumer_account.balance -= order.discount
                order.costumer_account.total_order_value -= order.discount
                order.costumer_account.save()
        order.status = Order_status.objects.get(id=7)
        order.save()
    if order.order_type == 'b':
        if int(order.paid_value) == 0:
            order.paid_value = order.value
            order.costumer_account.paid_value -= order.value
            order.costumer_account.total_order_value -= order.value
            order.costumer_account.save()
        order.status = Order_status.objects.get(id=8)
        order.save()
